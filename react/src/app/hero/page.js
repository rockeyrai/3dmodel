...
const CubeModle=()=> {
  return (
    ...
    <Canvas shadows gl={{ antialias: false }} camera={{ position: [30, 30, 30], fov: 25 }}>
      {/* <color attach="background" args={['black']} /> */}
      <ambientLight intensity={Math.PI / 2} />
      <spotLight position={[-10, 20, 20]} angle={0.15} penumbra={1} decay={0} intensity={2} castShadow />
      <Cubes gap={0.1} stride={5} displacement={4} intensity={2} />
      <EffectComposer>
...
export default CubeModle

function Cubes({ gap = 0.1, stride = 6, displacement = 7, intensity = 8 }) {
